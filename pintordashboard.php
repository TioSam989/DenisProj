...
<body>
    ...
    <section class="plan2" id="plan">
        ...
        $queryEspera = "SELECT COUNT(*) AS count FROM pintor WHERE status = 'em espera'";
        $queryAceitos = "SELECT COUNT(*) AS count FROM pintor WHERE status = 'aceito'";
        $queryRejeitados = "SELECT COUNT(*) AS count FROM pintor WHERE status = 'rejeitado'";
        ...
        ?>
            ...
            <div class="box">
                <br>
                <p><?php echo $totalEmEspera; ?></p>
                <p>emEspera</p>
            </div>

            <div class="box">
                <br>
                <p><?php echo $totalAceitos; ?></p>
                <p>Aceites</p>
            </div>

            <div class="box">
                <br>
                <p><?php echo $totalRejeitados; ?></p>
                <p>Rejeitadas</p>
            </div>
    ...
    if (isset($_POST['aprove'])) {
        $user = $_POST['user'];
        $result = mysqli_query($conn, "UPDATE pintor SET status  = 'aceito' WHERE id = $user;");
        echo '<script>window.location.href = "./pintordashboard.php";</script>';
        exit();
    }
    ...
    if (isset($_POST['reject'])) {
        $user = $_POST['user'];
        $result = mysqli_query($conn, "UPDATE pintor SET status  = 'rejeitado' WHERE id = $user;");
        echo '<script>window.location.href = "./pintordashboard.php";</script>';
        exit();
    }
